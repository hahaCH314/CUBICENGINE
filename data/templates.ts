...
const TEMPLATES: Tmpl[] = [
  ...
  {type:"ev_respawn", emoji:"HeartHandshake",label:"リスポーンしたとき",sublabel:"死んで復活した瞬間",       category:"trigger",fields:[]},
  {type:"ev_attack",  emoji:"Swords",label:"エンティティを攻撃",   sublabel:"モブや相手を殴ったとき",   category:"trigger",fields:[]},
  {type:"ev_kill",    emoji:"Skull",label:"敵をたおしたとき",      sublabel:"攻撃で相手を倒した瞬間",   category:"trigger",fields:[]},
  {type:"ev_useblock",emoji:"Hand",label:"ブロックを使用",        sublabel:"ブロックを右クリック",     category:"trigger",fields:[fv("block","ブロック","minecraft:chest",BLOCKS)]},
  {type:"ev_useentity",emoji:"Contact",label:"エンティティを使用",  sublabel:"モブを右クリック",         category:"trigger",fields:[]},
  {type:"ev_button",  emoji:"MousePointerClick",label:"ボタンを押したとき",sublabel:"ボタンを押した瞬間",       category:"trigger",fields:[]},
  {type:"ev_lever",   emoji:"ToggleLeft",label:"レバーを操作",     sublabel:"レバーを引いた/戻した",    category:"trigger",fields:[]},
  {type:"ev_hitblock",emoji:"Hammer",label:"ブロックを叩いた",     sublabel:"ブロックを左クリックで殴る",category:"trigger",fields:[fv("block","ブロック","minecraft:stone",BLOCKS)]},
  // ─── アクション ───
  ...
  {type:"ac_actionbar",emoji:"MonitorSmartphone",label:"アクションバー表示",sublabel:"画面下部に短く表示",   category:"action",fields:[fv("msg","メッセージ","こんにちは！",MSGS)]},
  {type:"ac_summon", emoji:"Ghost",label:"エンティティ召喚",    sublabel:"モブを足元に出現させる",   category:"action",fields:[fv("mob","エンティティ","minecraft:zombie")]},
  {type:"ac_particle",emoji:"Sparkle",label:"パーティクル表示",  sublabel:"エフェクト粒子を出す",     category:"action",fields:[fv("particle","粒子","minecraft:heart_particle")]},
  {type:"ac_xp",     emoji:"Star",label:"経験値を与える",      sublabel:"XPポイントを付与する",     category:"action",fields:[fv("amount","XP量","10",COUNTS)]},
  {type:"ac_heal",   emoji:"Cross",label:"HPを全回復",         sublabel:"体力を最大まで回復する",   category:"action",fields:[]},
  // ─── 条件 ───
  ...
  {type:"co_sprint",emoji:"Footprints",label:"ダッシュ中か",       sublabel:"走っているか確認",         category:"ifelse",fields:[]},
  {type:"co_water", emoji:"Droplets",label:"水中にいるか",         sublabel:"水に浸かっているか確認",   category:"ifelse",fields:[]},
  {type:"co_ground",emoji:"ArrowDownToLine",label:"地面にいるか",  sublabel:"接地しているか確認",       category:"ifelse",fields:[]},
  {type:"co_chance",emoji:"Dices",label:"確率で成立",             sublabel:"指定％の確率で真になる",   category:"ifelse",fields:[fv("pct","確率(%)","50")]},
  {type:"co_scoregte",emoji:"Trophy",label:"スコアが以上か",       sublabel:"スコアが基準以上か確認",   category:"ifelse",fields:[fv("obj","目標名","points",SCORE_OBJ),fv("val","基準","10")]},
  // ─── 値 ───
  ...
  {type:"va_rot",   emoji:"Compass",label:"向き（角度）",        sublabel:"体の向きYaw/Pitchを取得",  category:"value",  fields:[fv("axis","軸","Yaw",["Yaw","Pitch"])]},
  {type:"va_dim",   emoji:"Globe",label:"ディメンションID",      sublabel:"今いる世界のIDを取得",     category:"value",  fields:[]},
  {type:"va_level", emoji:"Star",label:"経験値レベル",          sublabel:"XPレベルを取得",           category:"value",  fields:[]},
  {type:"va_maxhp", emoji:"HeartPulse",label:"最大HP",           sublabel:"最大体力を取得",           category:"value",  fields:[]},
  {type:"va_count", emoji:"Users",label:"オンライン人数",        sublabel:"接続中のプレイヤー数",     category:"value",  fields:[]},
  {type:"va_time",  emoji:"Clock",label:"ワールド時間",          sublabel:"時刻（0〜24000）を取得",   category:"value",  fields:[]},
  {type:"va_bool",  emoji:"ToggleRight",label:"真偽値",          sublabel:"true/false の値",          category:"value",  fields:[fv("v","値","true",["true","false"])]},
  {type:"va_gamemode",emoji:"Gamepad2",label:"ゲームモード",     sublabel:"現在のモードを取得",       category:"value",  fields:[]},
  // ─── 制御 ───
  ...
  {type:"ct_return",emoji:"CircleStop",label:"中断",                sublabel:"これ以降の処理を止める",   category:"loop",   fields:[]},
  {type:"ct_delay", emoji:"Timer",label:"待機（ティック）",  sublabel:"指定ティックだけ待つ",     category:"loop",   fields:[fv("t","ティック","10")]},
  {type:"ct_waitrnd",emoji:"Shuffle",label:"ランダム待機",       sublabel:"最小〜最大秒のあいだ待つ", category:"loop",   fields:[fv("min","最小秒","1"),fv("max","最大秒","5")]},
  // ─── 演算：四則演算 ───
  ...
  {type:"vv_neq",  emoji:"EqualNot",label:"変数が違うか",       sublabel:"変数が指定値と違えば真",           category:"variable",fields:[fv("name","変数名","score"),fv("val","比較する値","0")]},
  {type:"vv_gte",  emoji:"ChevronsRight",label:"変数が以上か",     sublabel:"変数が指定値以上なら真",           category:"variable",fields:[fv("name","変数名","score"),fv("val","比較する値","0")]},
  {type:"vv_lte",  emoji:"ChevronsLeft",label:"変数が以下か",     sublabel:"変数が指定値以下なら真",           category:"variable",fields:[fv("name","変数名","score"),fv("val","比較する値","100")]},
  // ─── UI作成 ───
  ...
  {type:"ui_longtext",emoji:"BookOpen",label:"お知らせ表示",       sublabel:"長い説明を1つのボタンで表示",     category:"ui",
