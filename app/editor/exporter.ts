...
export async function exportBedrock(state: EditorState, jsCode: string) {
  ...
  const desc = `${escComment(state.projectName)} — Generated by MineModCraft Studio`;
  ...
  async function fillBP(bp: JSZip) {
    ...
    const mainJs = jsCode.trim()
      ? jsCode
      : [
          `// ${escComment(state.projectName)} — MineModCraft Studio`,
    ...
    bp.file("scripts/main.js", mainJs);
...
export async function exportJava(state: EditorState, jsCode: string) {
  ...
  zip.file(
    ...
    [
      `#!/bin/sh`,
      `# MineModCraft Studio 生成ビルドスクリプト`,
  ...
  zip.file(
    "gradlew.bat",
    [
      `@rem MineModCraft Studio 生成ビルドスクリプト`,
  ...
  metaInf.file(
    ...
    [
      ...
      `description='''${escComment(state.projectName)} — Generated by MineModCraft Studio'''`,
      ``,
      `[[dependencies.${modId}]]`,
      `    modId="forge"`,
      `    mandatory=true`,
  ...
  zip.file(
    ...
    [
      `# ${state.projectName}`,
      ``,
      `Generated by **MineModCraft Studio** on ${new Date().toLocaleDateString("ja-JP")}`,
      ``,
      `## Setup`,
      `1. Install **JDK 17+**`,
