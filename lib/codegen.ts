...
/* ══════════════════════════════════════════════════════════
   作品まるごと → 動くJS。
   もとは LogicPanel(UIコンポーネント)の中にあったが、共有ページ(/play)からも
   同じコードを作れないと「見た目は同じなのに出力が違う」事故が起きるので、
   生成の本体であるここへ移した。UI から呼ぶときは buildCode(blocks)。
   ══════════════════════════════════════════════════════════ */
export function buildCode(blocks: CBlock[]): string {
